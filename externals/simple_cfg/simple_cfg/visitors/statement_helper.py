...
def _get_names(node, result):
    ...
    if isinstance(node, ast.Name):
        ...
        if isinstance(node.value, ast.Name): # col slicing only
            return node.value.id
        if hasattr(node.value, 'attr'): # row slicing only
            return node.value.value.id
        elif hasattr(node.value.value, 'attr'):
            return node.value.value.value.id # extract variable name from 'iloc' statement (iloc WITH column slicing)
        else:
            return 'iloc'
...
def get_first_node(
    ...
    i = 0
    current_node = node
    while current_node.ingoing and current_node is not None:
