...
from ..translate.ast_helper import (Arguments, get_call_names_as_string)
from ..translate.module_definitions import ModuleDefinitions
...
from .statement_visitor import StmtVisitor
from .statement_helper import CALL_IDENTIFIER
...
class ExprVisitor(StmtVisitor):
    ...
    def init_cfg(self, node):
        ...
        if node == None:
            #logging.info("failed!! node is none : ", node)
            return
        ...
        if module_statements is None:
            return

        if module_statements.first_statement is None:
            return
